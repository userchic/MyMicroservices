...
namespace AuthService.Validators
{
    public class RegistryRequestValidator:AbstractValidator<RegistryRequest>
    {
        public RegistryRequestValidator()
        {
            ...
            RuleFor(request => request.Email).Must<RegistryRequest,string>((string email) => email.IsValidEmail() || string.IsNullOrEmpty(email)).WithMessage("Введенная строка не распознана как Email");
        }
    }
}
